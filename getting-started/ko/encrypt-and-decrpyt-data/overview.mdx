...
API 메서드(GET, POST, PATCH)와 WEBHOOK에서 데이터를 암호화하여 안전하게 전송합니다. API 메서드는 `AES-256-ECB` 방식을, WEBHOOK은 `AES-256-CBC` 방식을 사용하여 암호화합니다.
...
```javascript Node.js(crypto-js library)
    ...
    public class Encryptor {
        public static String encryptECB(String data, String apiKey) throws Exception {
            MessageDigest digest = MessageDigest.getInstance("SHA-256");
            byte[] hashedKey = digest.digest(apiKey.getBytes(StandardCharsets.UTF_8));
            
            SecretKeySpec key = new SecretKeySpec(hashedKey, 0, 32, "AES");
            Cipher cipher = Cipher.getInstance("AES/ECB/PKCS5Padding");
...
```javascript Node.js(crypto-js library)
  function decryptECB(encryptedData, apiKey) {
      const key = CryptoJS.SHA256(apiKey).toString(CryptoJS.enc.Hex);
...
public class Decryptor {
    public static String decryptECB(String encryptedData, String apiKey) throws Exception {
        MessageDigest digest = MessageDigest.getInstance("SHA-256");
        byte[] hashedKey = digest.digest(apiKey.getBytes(StandardCharsets.UTF_8));
...
public class Encryptor {
    public static String encryptCBC(String data, String apiKey) throws Exception {
        MessageDigest digest = MessageDigest.getInstance("SHA-256");
        byte[] hashedBytes = digest.digest(apiKey.getBytes(StandardCharsets.UTF_8));

        SecretKeySpec key = new SecretKeySpec(hashedBytes, 0, 32, "AES");
        IvParameterSpec iv = new IvParameterSpec(hashedBytes, 16, 16);
...
function decryptCBC(encryptedData, apiKey) {
    ...
    const decrypted = CryptoJS.AES.decrypt(
        ...
        key, { iv: iv, mode: CryptoJS.mode.CBC }
    );
    return JSON.parse(decrypted.toString(CryptoJS.enc.Utf8));
}
