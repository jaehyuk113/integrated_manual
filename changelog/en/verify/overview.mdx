...
<Update label="2025-06-26">
  ## Textify ID AutoParser API Release

   Textify ID AutoParser API has been added! It automatically extracts structured data from government ID documents, eliminating manual data entry and accelerating identity verification workflows.
...
<Update label="2025-06-17">
  ## Validate CURP API Release

   The Validate CURP API has been added! Use the API to validate the CURP number, Mexico's population registration code, using the Registro Nacional de Población (RENAPO) database hosted by the official Mexican government.

    Check out the link for more information\!

  - [Validate CURP API Documentation](https://app.theneo.io/argosidentity/verifier/validate-curp-api/validate-curp-api-1)
  - [Test the API](https://app.theneo.io/argosidentity/verifier/validate-curp-api/validate-curp-api-1)
