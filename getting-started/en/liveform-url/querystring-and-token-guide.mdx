...
<AccordionGroup>
  ...
  <Accordion title="[ID document] allowedIdTypes" icon="notebook">
    - `passport`
    - `drivers_license`
    - `government_id`
    - `residence_permit`
  ...
  <Accordion title="[ID document] allowedDocumentFields" icon="notebook">
    - `gender`
    - `nationality`
    - `expiry_date`
    - `number`
...
In the example above, the query string exact option is`{'knowledgeField': 'gender,nationality,name'}`
