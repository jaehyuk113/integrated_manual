...
| `allowedCountries`                                | Yes         | Yes             | Must be encrypted.  `allowedCountries` can only be set using the encryption option and cannot be specified directly. e.g., If the encrypted value `{'allowedCountries': 'USA,KOR'}` is provided, only the United States (USA) and South Korea (KOR) will be allowed.  - In Knowledge-Based, if `allowedCountries` set one nationality, e.g. `{'allowedCountries': 'USA'}` The form skips to the select country page. |
| `knowledgeField`     Required encrypted parameter | No          | Yes             | Must be encrypted.  Set visible fields from the supported fields. Refer to the value below each parameter.                                                                                                                                                                                                                                                                                                           |
| `knowledgePrefill`   Required encrypted parameter | No          | Yes             | Must be encrypted.  Fill out the value in advance. Refer to the value below each parameter.                                                                                                                                                                                                                                                                                                                          |

### 2-1-a. Knowledge-Based Parameter Values

<AccordionGroup>
  <Accordion title="knowledgeField Parameter Values" icon="memo-pad">
    - `name`
    - `gender`
    - `birthDate`
    - `nationality`
  ...
  <Accordion title="knowledgePrefill Parameter Values" icon="memo-pad">
    - `name` Input name
