...
| `allowedCountries`  Required encrypted parameter  | Yes         | Yes             | `allowedCountries` can only be set using the encryption option and cannot be specified directly. e.g., If the encrypted value `{'allowedCountries': 'USA,KOR'}` is provided, only the United States (USA) and South Korea (KOR) will be allowed.  - In Knowledge-Based, if `allowedCountries` set one nationality, e.g. `{'allowedCountries': 'USA'}` The form skips to the select country page. |
