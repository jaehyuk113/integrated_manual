...
<Update label="2025-08-01">
  # Mid-Session Resume Bug Fix

  Previously, when a user entered a new QR code after abandoning a session, the system resumed the previous project instead. This has been fixed: entering a new QR code will no longer resume the previously abandoned project.
  ...
  # Auto-English Name Conversion Enhancement

  For auto-English name conversion, `name_en` is now prioritized over `name` in Step 1 responses. For KR passports, `name` will only be prioritized when the "Government Authenticity Check" option is enabled.
</Update>
...
<Update label="2025-07-28">
  ...
  # Smart ID Engine Update

  Updated Smart ID engine to support 95 ID documents from 46 countries, including 6 new PHL ID documents that were not previously supported.
...
<Update label="2025-07-25">
  # CodeF Certificate Issue Resolution

  Resolved an issue where Korean ID authenticity checks were failing due to an expired CodeF certificate.

  # MRZ Verification Logic Update

  Updated MRZ verification logic so that only missing fields are bypassed, rather than bypassing the entire MRZ check when a single field is missing. This ensures more accurate argosScore calculation.
</Update>
...
<Update label="2025-06-20">
  ...
  # Liveness Exception for Peru

  Liveness (paper printed) on PER.id (Peru national ID card) was applied as an exception.
</Update>
...
<Update label="2025-06-02">
  ...
  Added Return URL functionality. If the submitter clicks the "Ok" button after the KYC process is complete, the URL will be redirected based on the advanced set parameters. Location: Dashboard > Project Info > Return URL.

  # LLM Engine Update

  Updated LLM Engine (GPT 3.5 → GPT 4.1-mini). GPT is the engine that supports recognition, and the update has made it work better.
</Update>
