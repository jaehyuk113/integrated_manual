...
<ResponseField name="ipRiskDurationHours" type="int">
  **암호화가 적용된 Injection 포함 URL에서만 지원.** 위험이 탐지되었을 때 제한할 시간을 설정할 수 있습니다. 반드시 `ipRiskCategory` 옵션과 함께 사용해야 됩니다.
</ResponseField>
...
IP risk 탐지에 관해선 별도의 Webhook 은 없습니다. 다만 거리 확인 혹은 블랙리스트 국가 관련 Fail 이 발생된 경우 다음과 같이 Webhook 을 발송합니다.
